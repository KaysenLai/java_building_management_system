...
public class CarbonDioxideSensor extends TimedSensor
     ...
     * given by: the (absolute) difference between
     * the ideal CO2 value and the current sensor reading, all divided by
     * getVariationLimit().
     *
     ...
     * For example:
     *
     * ·a sensor with an ideal CO2 value of 600ppm, a variation limit of 200ppm,
     *      and a current sensor reading of 640ppm would have a comfort level
     *      of 80
     * ·a sensor with an ideal CO2 value of 500ppm, a variation limit of 100ppm,
     *      and a current sensor reading of 420ppm would have a comfort level
     *      of 20
     * ·a sensor with an ideal CO2 value of 800ppm, a variation limit of 100ppm,
     *      and a current sensor reading of 1000ppm would have a comfort level
     *      of 0
     *
    ...
    public int getComfortLevel() {
        final int idealValue = this.getIdealValue();
        final int variationLimit = this.getVariationLimit();
        final int currentReading = this.getCurrentReading();
        final double absoluteDiff = Math.abs(currentReading - idealValue);
        final double result = (1.0 - absoluteDiff / variationLimit) * 100;
        return result < 0 ? 0 : (int) Math.round(result);
    }
    ...
    public boolean equals(Object obj) {
        CarbonDioxideSensor castObj = (CarbonDioxideSensor) obj;
        return super.equals(obj) &&
                this.getIdealValue() == castObj.getIdealValue() &&
                this.getVariationLimit() == castObj.getVariationLimit();
    }

    /**
     * Returns the hash code of this CO2 sensor.
     * Two CO2 sensors that are equal according to equals(Object) should
    ...
    @Override
    public int hashCode() {
        final int typePrime = 3;
        int result = super.hashCode();

        result = typePrime * result + idealValue;
        result = typePrime * result + variationLimit;
        return result;
    }
    ...
    public String encode() {
        return String.join(":",
                ...
                super.encode(),
                String.valueOf(super.getUpdateFrequency()),
                String.valueOf(getIdealValue()),
                String.valueOf(getVariationLimit())
        );
    }
}
