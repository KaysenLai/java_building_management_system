...
public class Building implements FireDrill, Encodable {
     ...
     * dimensions small enough so that this reduced floor can still
     * support them.
     * <p>
     ...
     *          (i.e. getFloorByNumber() is null), or if
     ...
     *          support increased dimensions, if the floor above is too large
     *          to be supported by decreased dimensions, or if the total size
     *          of the current rooms could not be supported by decreased
     *          dimensions
     */
    public void renovateFloor(int floorNumber, double newWidth, double newLength)
        ...
        if (floorNumber >= 2 && (newWidth > floorBelow.getWidth()
                || newLength > floorBelow.getLength())) {
            throw new FloorTooSmallException("The floor below is too small to " +
                    "support increased dimensions.");
        }
        ...
        if (floorAbove != null && (newWidth < floorAbove.getWidth()
            ...
            throw new FloorTooSmallException("The floor above is too large to " +
                    "be supported by decreased dimensions.");
        }

        this.getFloorByNumber(floorNumber).changeDimensions(newWidth,newLength);
    }
    ...
    public boolean equals(Object obj) {
        ...
        Building castObj = (Building) obj;

        for(Floor floor : this.getFloors()) {
            int floorNumber = floor.getFloorNumber();
            Floor objFloor = castObj.getFloorByNumber(floorNumber);
            if (!floor.equals(objFloor))
                return false;
        }

        return this.getName().equals(castObj.getName()) &&
                this.getFloors().size() == castObj.getFloors().size();
    }

    /**
     * Returns the hash code of this building.
     * Two buildings that are equal according to equals(Object)
    ...
    public int hashCode() {
        final int prime = 11;
        int floorsHashCode = 0;

        for(Floor floor : this.getFloors()) {
            floorsHashCode += floor.hashCode();
        }

        int result = 1;
        result = result * prime + getFloors().size();
        result = result * prime + getName().hashCode();
        result = result * prime + floorsHashCode;
    ...
    public String encode() {
        ...
        List<Floor> floors = this.getFloors();

        String buildingEncode = String.join(System.lineSeparator(),
                this.getName(),
                String.valueOf(floors.size())
        );

        for (Floor floor : floors) {
            buildingEncode = String.join(System.lineSeparator(),
                    buildingEncode, floor.encode());
        }

        return buildingEncode;
    }
}
